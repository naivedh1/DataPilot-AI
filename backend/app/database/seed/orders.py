...
def _pick_status(
    ...
    Return likelihood is biased by the basket's dominant category and lifted
    further inside a return-rate anomaly window.
    """
    ...
    return_bias = CATEGORY_RETURN_BIAS.get(dominant_category, 1.0)
    return_bias *= anomaly_rules.return_rate_multiplier(dominant_category, day)

    weights = dict(RESOLVED_STATUS_WEIGHTS)
    weights["returned"] *= return_bias
    keys = list(weights)
...
def build_orders(
...
) -> tuple[list[tuple[object, ...]], list[tuple[object, ...]], GenProduct]:
    """Generate all orders and their lines.

    Returns `(order_rows, order_item_rows, collapsed_product)`.
    """
    ...
    return order_rows, item_rows, collapsed_product
